...
app.post('/api/orders', verifyToken, async (req, res) => {
  try {
   ...
   if (product.is499k) {
      ...
      // 🔥 ยิงไปที่ 499K ก่อนเลยอันดับแรก!
      ...
      // 🔥 ถ้า 499K ตีกลับว่าพัง / เต็ม / หรือ Error ใดๆ ให้ดีดออกทันที (เงินลูกค้าจะไม่ถูกหักสักบาท!)
      if (!apiData.success) {
        if (apiData.error?.code === 'INSUFFICIENT_BALANCE') {
          sendDiscordAlert("🚨 ฉุกเฉิน! เงินทุน 499K หมด!", `บอสครับ! ลูกค้าชื่อ ${user.username} พยายามซื้อ **${productName}** แต่เงินทุนในเว็บ 499K ไม่พอตัด!`, 16711680); 
    ...
    // 🔥 พอยืนยันแล้วว่า 499K หรือคีย์พร้อมใช้งานจริงๆ ค่อยทำการหักเงินและบันทึกลงฐานข้อมูล!
    user.balance -= price;
    await user.save();
...
// ==========================================
// 🛡️ API ขอโค้ด Steam Guard จาก 499K แบบไม่ต้องกรอกเหตุผล
// ==========================================
app.post('/api/orders/:id/steam-guard', verifyToken, async (req, res) => {
  try {
    const order = await Order.findOne({ _id: req.params.id, userId: req.userId });
    if (!order || !order.apiOrderNo) {
      return res.status(404).json({ message: 'ไม่พบรหัสอ้างอิงสำหรับขอโค้ด (ออเดอร์เก่าจะขอไม่ได้)' });
    }

    const response = await fetch(`https://store.499k-network.com/api/v1/orders/${order.apiOrderNo}/code`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${process.env.API_499K_KEY}` },
      body: JSON.stringify({ reason: 'ลูกค้าขอโค้ดผ่านหน้าเว็บเพื่อเข้าสู่ระบบ' }) // แอบส่งเนียนๆ ไปเลย
    });

    const apiData = await response.json();

    if (!apiData.success) {
      return res.status(400).json({ message: apiData.error?.message || 'ไม่สามารถดึงโค้ดได้' });
    }

    res.json({ success: true, code: apiData.data.code });
  } catch (err) {
    res.status(500).json({ message: 'เซิร์ฟเวอร์ขัดข้องตอนดึงโค้ด' });
  }
});
...
async function initializeData() {
  ...
  if (count === 0) {
    await Product.insertMany([
      ...
      { name: 'Macro ALLMOUSE', category: 'PUBG PC', price: 119, description: 'สามารถใช้ได้กับเมาส์ทุกชนิด และมีตั้งค่าสำหรับDPI 400/800/1600', badge: 'HOT', image: '/images/2.gif', soldCount: 74 },
